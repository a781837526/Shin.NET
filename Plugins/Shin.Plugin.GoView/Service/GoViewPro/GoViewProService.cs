...
public class GoViewProService : IGoViewProService
{
    private readonly ISqlSugarRepository<GoViewPro> _repository;

    public GoViewProService(ISqlSugarRepository<GoViewPro> repository)
    {
    ...
    {
        var project = await _repository.AsInsertable(input.Adapt<GoViewPro>()).ExecuteReturnEntityAsync();
    ...
    {
        await _repository.AsUpdateable(input.Adapt<GoViewPro>()).IgnoreColumns(true).ExecuteCommandAsync();
    }
    ...
    public async Task Delete([FromQuery] string ids)
    {
        ...
        await _repository.AsDeleteable().Where(u => idList.Contains(u.Id)).ExecuteCommandAsync();
        await _repository.Context.Deleteable<GoViewProData>().Where(u => idList.Contains(u.Id)).ExecuteCommandAsync();
    }
    ...
    {
        var projectData = await _repository.Context.Queryable<GoViewProData>().FirstAsync(it => it.Id.Equals(projectId));
    ...
    {
        if (await _repository.Context.Queryable<GoViewProData>().AnyAsync(u => u.Id.Equals(input.ProjectId)))
        {
            await _repository.Context.Updateable<GoViewProData>()
        ...
        {
            await _repository.Context.Insertable(new GoViewProData
            {
                Id = input.ProjectId,
                Content = input.Content,
            }).ExecuteCommandAsync();
    ...
    public async Task<GoViewProUploadOutput> Upload(IFormFile @object)
    {
        ...
        // 保存
        if (await _repository.Context.Queryable<GoViewProData>().AnyAsync(u => u.Id == id))
        {
            await _repository.Context.Updateable<GoViewProData>()
                .SetColumns(u => new GoViewProData
                {
    ...
    {
        var projectData = await _repository.Context.Queryable<GoViewProData>().IgnoreColumns(u => u.Content).FirstAsync(u => u.Id == id);
