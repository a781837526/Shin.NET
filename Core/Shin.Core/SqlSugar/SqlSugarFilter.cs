...
public static class SqlSugarFilter
{
    ...
    private static readonly ISysOrgService SysOrgService = App.GetRequiredService<ISysOrgService>();
    private static readonly ICacheManager SysCacheService = App.GetRequiredService<ICacheManager>();
    ...
    private static int SetDataScopeFilter(SqlSugarScopeProvider db)
    {
        ...
        // 获取用户最大数据范围---仅本人数据
        maxDataScope = App.GetRequiredService<ICacheManager>().Get<int>(CacheConst.KeyRoleMaxDataScope + userId);
        // 若为0则获取用户机构组织集合建立缓存
        if (maxDataScope == 0)
        {
            // 获取用户所属机构，保证同一作用域
