...
public class SysDatabaseService : ISysDatabaseService
{
    ...
    /// <summary>
    /// 初始化<see cref="SysDatabaseService"/>类的新实例
    /// </summary>
    ...
    {
        var config = App.GetOptions<DbConnectionOptions>().ConnectionConfigs.FirstOrDefault(u => u.ConfigId.ToString() == input.ConfigId);
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "Shin.Application" : input.Position;
        ...
        {
            Assembly assembly = Assembly.Load("Shin.Core");
            Type type = assembly.GetType($"Shin.Core.{input.BaseClassName}");
            if (type is null)
    ...
    {
        var config = App.GetOptions<DbConnectionOptions>().ConnectionConfigs.FirstOrDefault(u => u.ConfigId.ToString() == input.ConfigId);
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "Shin.Core" : input.Position;
