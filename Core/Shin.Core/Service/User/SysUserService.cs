...
public class SysUserService : ISysUserService
{
    ...
    /// 用户域账号服务
    /// </summary>
    private readonly ISysUserLdapService _sysUserLdapService;
    ...
    public SysUserService(ISqlSugarRepository<SysUser> repository,
        ...
        ISysUserLdapService sysUserLdapService,
        ISysUserMenuService sysUserMenuService,
    ...
    {
        var isExist = await _repository.AsQueryable().ClearFilter().AnyAsync(u => u.Account == input.Account);
        if (isExist) throw Oops.Oh(ErrorCodeEnum.D1003);

        if (!string.IsNullOrWhiteSpace(input.Phone) && await _repository.AsQueryable().ClearFilter().AnyAsync(u => u.Phone == input.Phone))
        ...
        var newUser = await _repository.AsInsertable(user).ExecuteReturnEntityAsync();
    ...
    {
        if (await _repository.AsQueryable().ClearFilter().AnyAsync(u => u.Account == input.Account && u.Id != input.Id))
            throw Oops.Oh(ErrorCodeEnum.D1003);

        if (!string.IsNullOrWhiteSpace(input.Phone) && await _repository.AsQueryable().ClearFilter().AnyAsync(u => u.Phone == input.Phone && u.Id != input.Id))
            throw Oops.Oh(ErrorCodeEnum.D1032);

        // 禁止越权更新超级管理员或系统管理员信息
        if (_userManager.AccountType != AccountTypeEnum.SuperAdmin && input.AccountType is AccountTypeEnum.SuperAdmin or AccountTypeEnum.SysAdmin) throw Oops.Oh(ErrorCodeEnum.D1038);

        await _repository.AsUpdateable(input.Adapt<SysUser>()).IgnoreColumns(true)
            .IgnoreColumns(u => new { u.Password, u.Status, u.TenantId }).ExecuteCommandAsync();

        await UpdateRoleAndExtOrg(input);

        // 删除用户机构缓存
        SqlSugarFilter.DeleteUserOrgCache(input.Id, _repository.Context.CurrentConnectionConfig.ConfigId.ToString());

        // 若账号的角色和组织架构发生变化,则强制下线账号进行权限更新
        var user = await _repository.AsQueryable().ClearFilter().FirstAsync(u => u.Id == input.Id);
    ...
    {
        var user = await _repository.AsQueryable().ClearFilter().FirstAsync(u => u.Id == _userManager.UserId) ?? throw Oops.Oh(ErrorCodeEnum.D1011).StatusCode(401);
        user.LangCode = langCode;
        await _repository.AsUpdateable(user).UpdateColumns(it => it.LangCode).ExecuteCommandAsync();
    }
    ...
    {
        var user = await _repository.GetByIdAsync(input.Id) ?? throw Oops.Oh(ErrorCodeEnum.D0009);
        user.ValidateIsSuperAdminAccountType();
        user.ValidateIsUserId(_userManager.UserId);

        // 若账号为租户默认账号则禁止删除
        var isTenantUser = await _repository.ChangeRepository<SqlSugarRepository<SysTenant>>().IsAnyAsync(u => u.UserId == input.Id);
        if (isTenantUser) throw Oops.Oh(ErrorCodeEnum.D1029);

        // 若账号为开放接口绑定账号则禁止删除
        var isOpenAccessUser = await _repository.ChangeRepository<SqlSugarRepository<SysOpenAccess>>().IsAnyAsync(u => u.BindUserId == input.Id);
    ...
    {
        return await _repository.AsQueryable().ClearFilter().FirstAsync(c => c.Id == _userManager.UserId);
    }
    ...
    {
        return await _repository.AsUpdateable(user)
    ...
    public virtual async Task<int> SetStatus(UserInput input)
    {
        ...
        var cacheManager = App.GetRequiredService<ICacheManager>();
        if (input.Status == StatusEnum.Disable)
        {
            cacheManager.Set($"{CacheConst.KeyBlacklist}{user.Id}", $"{user.RealName}-{user.Phone}");
        ...
        {
            cacheManager.Remove($"{CacheConst.KeyBlacklist}{user.Id}");
        }

        user.Status = input.Status;
        var rows = await _repository.AsUpdateable(user).UpdateColumns(u => new { u.Status }).ExecuteCommandAsync();
    ...
    public virtual async Task<int> ChangePwd(ChangePwdInput input)
    {
        ...
        var user = await _repository.AsQueryable().ClearFilter().FirstAsync(c => c.Id == _userManager.UserId) ?? throw Oops.Oh(ErrorCodeEnum.D0009);
        ...
        var rows = await _repository.AsUpdateable(user).UpdateColumns(u => u.Password).ExecuteCommandAsync();
    ...
    public virtual async Task<string> ResetPwd(ResetPwdUserInput input)
    {
        ...
        await _repository.AsUpdateable(user).UpdateColumns(u => u.Password).ExecuteCommandAsync();

        // 清空密码错误次数
        var keyErrorPasswordCount = $"{CacheConst.KeyPasswordErrorTimes}{user.Account}";
        _cacheManager.Remove(keyErrorPasswordCount);
    ...
    public virtual async Task UnlockLogin(UnlockLoginInput input)
    {
        ...
        _cacheManager.Remove(keyPasswordErrorTimes);
