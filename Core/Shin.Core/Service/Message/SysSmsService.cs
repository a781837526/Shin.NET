...
public class SysSmsService : ISysSmsService
{
    /// <summary>
    /// 短信配置选项
    /// </summary>
    ...
    /// <summary>
    /// 初始化<see cref="SysSmsService"/>类的新实例
    /// </summary>
    ...
    {
        var verifyCode = _cacheManager.Get<string>($"{CacheConst.KeyPhoneVerCode}{input.Phone}");
    ...
    public async Task AliyunSendSms([Required] string phoneNumber, string templateId = "0")
    {
        ...
        if (sendSmsResponse.Body.Code == "OK" && sendSmsResponse.Body.Message == "OK")
        {
            ...
            _cacheManager.Set($"{CacheConst.KeyPhoneVerCode}{phoneNumber}", verifyCode, TimeSpan.FromSeconds(_smsOptions.VerifyCodeExpireSeconds));
        }
        else
        {
