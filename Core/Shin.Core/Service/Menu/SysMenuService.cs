...
public class SysMenuService : ISysMenuService
{
    ...
    /// 翻译服务
    /// </summary>
    private readonly ISysLangTextService _sysLangTextService;

    /// <summary>
    /// 初始化<see cref="SysMenuService"/>类的新实例
    /// </summary>
    public SysMenuService(
        ISqlSugarRepository<SysMenu> repository,
        ...
        ISysLangTextService sysLangTextService)
    {
    ...
    public async Task<long> AddMenu(AddMenuInput input)
    {
        ...
        // 保存租户菜单权限
        await _repository.InsertAsync(sysMenu);
        await _repository.Context.Insertable(new SysTenantMenu { TenantId = tenantId, MenuId = sysMenu.Id }).ExecuteCommandAsync();
    ...
    public async Task UpdateMenu(UpdateMenuInput input)
    {
        ...
        {
            // 更新菜单
            await _repository.AsUpdateable(sysMenu).ExecuteCommandAsync();
    ...
    public async Task DeleteMenu(DeleteMenuInput input)
    {
        ...
        var menuTreeList = await _repository.AsQueryable().ToChildListAsync(u => u.Pid, input.Id);
        var menuIdList = menuTreeList.Select(u => u.Id).ToList();

        await _repository.DeleteAsync(u => menuIdList.Contains(u.Id));

        // 级联删除租户菜单数据
        await _repository.Context.Deleteable<SysTenantMenu>().Where(u => menuIdList.Contains(u.MenuId)).ExecuteCommandAsync();
    ...
    public virtual async Task<int> SetStatus(MenuStatusInput input)
    {
        ...
        var menu = await _repository.GetByIdAsync(input.Id) ?? throw Oops.Oh(ErrorCodeEnum.D1002);
        menu.Status = input.Status;
        var rows = await _repository.AsUpdateable(menu).UpdateColumns(u => new { u.Status }).ExecuteCommandAsync();
        return rows;
    }
    ...
    public async Task<List<string>> GetOwnBtnPermList()
    {
        ...
        var permissions = _cacheManager.Get<List<string>>(CacheConst.KeyUserButton + userId);
        ...
        {
            //_cacheManager.Set(CacheConst.KeyUserButton + userId, new List<string>(), TimeSpan.FromDays(7));
            return new List<string>();
        }

        permissions = await _repository.AsQueryable()
            .InnerJoinIF<SysTenantMenu>(!_userManager.SuperAdmin, (u, t) => t.TenantId == _userManager.TenantId && u.Id == t.MenuId)
            .Where(u => u.Type == MenuTypeEnum.Btn)
            .WhereIF(menuIdList.Count > 0, u => menuIdList.Contains(u.Id))
            .Select(u => u.Permission).ToListAsync();

        _cacheManager.Set(CacheConst.KeyUserButton + userId, permissions, TimeSpan.FromDays(7));
    ...
    {
        var permissions = _cacheManager.Get<List<string>>(CacheConst.KeyUserButton + 0);
        if (permissions != null && permissions.Count != 0) return permissions;

        permissions = await _repository.AsQueryable()
            .Where(u => u.Type == MenuTypeEnum.Btn)
            .Select(u => u.Permission).ToListAsync();
        _cacheManager.Set(CacheConst.KeyUserButton + 0, permissions);
    ...
    public (ISugarQueryable<SysMenu, SysTenantMenu> query, long tenantId) GetSugarQueryableAndTenantId(long tenantId)
    {
        ...
        if (_userManager.SuperAdmin)
        {
            if (tenantId <= 0)
            {
                query = _repository.AsQueryable().InnerJoinIF<SysTenantMenu>(false, (u, t) => true);
            }
            else
            {
                // 指定租户的菜单
                var menuIds = _repository.Context.Queryable<SysTenantMenu>().Where(u => u.TenantId == tenantId).ToList(u => u.MenuId) ?? new();

                // 种子菜单
                //menuIds.AddRange(new SysMenuSeedData().HasData().Select(u => u.Id).ToList());

                menuIds = menuIds.Distinct().ToList();
                query = _repository.AsQueryable().InnerJoinIF<SysTenantMenu>(false, (u, t) => true).Where(u => menuIds.Contains(u.Id));
            }
        }
        else
        {
            query = _repository.AsQueryable().InnerJoinIF<SysTenantMenu>(tenantId > 0, (u, t) => t.TenantId == tenantId && u.Id == t.MenuId);
        }
