...
public class SysOAuthService : ISysOAuthService
{
    /// <summary>
    /// Http上下文
    /// </summary>
    ...
    /// <summary>
    /// 初始化<see cref="SysOAuthService"/>类的新实例
    /// </summary>
    public SysOAuthService(IHttpContextAccessor httpContextAccessor,
        ISqlSugarRepository<SysWechatUser> sysWechatUserRep)
    {
        _httpContextAccessor = httpContextAccessor;
        _repository = sysWechatUserRep;
    }
    ...
    public virtual async Task<IActionResult> SignInCallback([FromQuery] string provider = null, [FromQuery] string redirectUrl = "")
    {
        ...
        // 若账号不存在则新建
        var wechatUser = await _repository.AsQueryable().Includes(u => u.SysUser).ClearFilter().FirstAsync(u => u.OpenId == openIdClaim.Value);
        if (wechatUser == null)
        {
            ...
            await _repository.InsertAsync(new SysWechatUser()
            {
                UserId = userId,
            ...
            wechatUser = await _repository.AsQueryable().Includes(u => u.SysUser).ClearFilter().FirstAsync(u => u.OpenId == openIdClaim.Value);
        }
