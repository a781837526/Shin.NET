...
/// 系统文件存储提供者服务 🧩
/// </summary>
[ApiDescriptionSettings(Order = 411, Description = "文件存储提供者", IgnoreApi = true)]
public class SysFileProviderService : ISysFileProviderService
{
    /// <summary>
    /// 基础仓储服务
    /// </summary>
    private readonly ISqlSugarRepository<SysFileProvider> _repository;
    ...
    /// <summary>
    /// 初始化<see cref="SysFileProviderService"/>类的新实例
    /// </summary>
    public SysFileProviderService(ISqlSugarRepository<SysFileProvider> repository,
    ...
    public async Task AddFileProvider(AddFileProviderInput input)
    {
        ...
        var isExist = await _repository.AsQueryable()
        ...
        await _repository.InsertAsync(fileProvider);
    ...
    public async Task BatchEnableProvider(BatchEnableProviderInput input)
    {
        ...
        {
            var provider = await GetFileProviderById(id);
            if (provider != null)
            {
                ...
                await UpdateFileProvider(updateInput);
            }
        }
    }
    ...
    public async Task UpdateFileProvider(UpdateFileProviderInput input)
    {
        ...
        await _repository.AsUpdateable(fileProvider).IgnoreColumns(ignoreAllNullColumns: true).ExecuteCommandAsync();
    ...
    {
        // 检查是否为默认提供者
        var provider = await _repository.GetByIdAsync(input.Id) ?? throw Oops.Oh("存储提供者不存在").StatusCode(400);
        var isDefault = provider.IsDefault == true;

        await _repository.DeleteByIdAsync(input.Id);

        // 如果删除的是默认提供者，自动设置第一个启用的提供者为默认
        if (isDefault)
        {
            var firstEnabledProvider = await _repository.AsQueryable()
    ...
    public async Task SetDefaultProvider(SetDefaultProviderInput input)
    {
        ...
        // 开启事务，确保数据一致性
        await _repository.AsTenant().BeginTranAsync();
        try
        {
            // 先将所有提供者的默认标识设为false
            await _repository.AsUpdateable()
            ...
            await _repository.AsTenant().CommitTranAsync();
        ...
        {
            await _repository.AsTenant().RollbackTranAsync();
            throw;
        }
    }
    ...
    {
        return await _cacheManager.AdGetAsync(CacheKey, async () =>
        {
            return await _repository.AsQueryable()
                .Where(u => u.IsEnable == true)
                .OrderBy(u => u.OrderNo)
    ...
    private async Task HandleDefaultProviderLogic(SysFileProvider provider)
    {
        ...
        var hasDefaultProvider = await _repository.AsQueryable()
