...
public class SysFileService : ISysFileService
{
    ...
    /// 基础服务仓储
    /// </summary>
    private readonly ISqlSugarRepository<SysFile> _repository;
    ...
    /// <summary>
    /// 初始化<see cref="SysFileService"/>类的新实例
    /// </summary>
    public SysFileService(IUserManager userManager,
        ISqlSugarRepository<SysFile> repository,
    ...
    {
        // 获取所有公开附件
        var publicList = _repository.AsQueryable().ClearFilter().Where(u => u.IsPublic == true);
        // 获取私有附件
        var privateList = _repository.AsQueryable().Where(u => u.IsPublic == false);
        // 合并公开和私有附件并分页
        return await _repository.Context.UnionAll(publicList, privateList)
    ...
    {
        var file = input.Id > 0 ? await GetFile(input.Id) : await _repository.CopyNew().GetFirstAsync(u => u.Url == input.Url);
    ...
    {
        var sysFile = await _repository.CopyNew().GetFirstAsync(u => u.Url == url) ?? throw Oops.Oh($"文件不存在");
    ...
    {
        var file = await _repository.GetByIdAsync(input.Id) ?? throw Oops.Oh($"文件不存在");
    ...
    {
        var isExist = await _repository.IsAnyAsync(u => u.Id == input.Id);
    ...
    {
        var file = await _repository.CopyNew().GetByIdAsync(id);
    ...
    {
        return await _repository.AsQueryable().Where(u => ids.Contains(u.Id)).ToListAsync();
    }
    ...
    public async Task<List<TreeNode>> GetFolder()
    {
        ...
        var folders = await _repository.AsQueryable()
            .Select(u => u.FilePath)
            .Distinct()
            .ToListAsync();
    ...
    public async Task<SysFile> UploadFile([FromForm] UploadFileInput input, [BindNever] string targetPath = "")
    {
        ...
        if (_uploadOptions.EnableMd5)
        {
            ...
            var sysFile = await _repository.GetFirstAsync(u => u.FileMd5 == fileMd5 && u.SizeKb == sizeKb);
    ...
    public async Task<SysFile> UploadAvatar([Required] IFormFile file)
    {
        ...
        var sysUserRep = _repository.ChangeRepository<SqlSugarRepository<SysUser>>();
    ...
    public async Task UpdateFileByDataId(long dataId, List<SysFile> sysFiles)
    {
        ...
        // 求文件Id差集并删除（无效文件）
        var tmpFiles = await _repository.GetListAsync(u => u.DataId == dataId);
        ...
        await _repository.UpdateAsync(u => new SysFile() { DataId = dataId }, u => newFileIds.Contains(u.Id));
    }
    ...
    public async Task DeleteFileByDataId(long dataId)
    {
        ...
        foreach (var file in tmpFiles)
            await _customFileProvider.DeleteFileAsync(file);
        await _repository.AsDeleteable().Where(u => u.DataId == dataId).ExecuteCommandAsync();
    }
