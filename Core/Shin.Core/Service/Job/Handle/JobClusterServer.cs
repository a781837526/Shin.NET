...
public class JobClusterServer : IJobClusterServer
{
    ...
    /// 基础仓储服务
    /// </summary>
    private static readonly ISqlSugarRepository<SysJobCluster> _repository = App.GetRequiredService<ISqlSugarRepository<SysJobCluster>>();
    ...
    public async void Start(JobClusterContext context)
    {
        ...
        if (await _repository.IsAnyAsync(u => u.ClusterId == context.ClusterId))
        {
            await _repository.AsUpdateable().SetColumns(u => u.Status == ClusterStatus.Waiting).Where(u => u.ClusterId == context.ClusterId).ExecuteCommandAsync();
        }
        else
        {
            await _repository.AsInsertable(new SysJobCluster { ClusterId = context.ClusterId, Status = ClusterStatus.Waiting }).ExecuteCommandAsync();
        }
    }
        ...
        while (true)
        {
            ...
            try
            {
                ...
                using (cache.AcquireLock("lock:JobClusterServer:WaitingForAsync", 1000))
                {
                    ...
                    if (await _repository.IsAnyAsync(u => u.Status == ClusterStatus.Working)) continue;
    ...
    public async void Stop(JobClusterContext context)
    {
        ...
        await _repository.UpdateAsync(u => new SysJobCluster { Status = ClusterStatus.Crashed }, u => u.ClusterId == context.ClusterId);
    }
    ...
    private static async Task WorkNowAsync(string clusterId)
    {
        ...
        await _repository.UpdateAsync(u => new SysJobCluster { Status = ClusterStatus.Working }, u => u.ClusterId == clusterId);
    }
}