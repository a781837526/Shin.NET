...
public partial class WechatApiClientFactory : ISingleton
{
    ...
    {
        if (!_cacheManager.ExistKey($"WxAccessToken_{_wechatOptions.WechatAppId}") || string.IsNullOrEmpty(_cacheManager.Get<string>($"WxAccessToken_{_wechatOptions.WechatAppId}")))
        {
            ...
            _cacheManager.Set($"WxAccessToken_{_wechatOptions.WechatAppId}", resCgibinToken.AccessToken, TimeSpan.FromSeconds(resCgibinToken.ExpiresIn - 60));
        }

        return _cacheManager.Get<string>($"WxAccessToken_{_wechatOptions.WechatAppId}");
    }
    ...
    {
        if (!_cacheManager.ExistKey($"WxAccessToken_{_wechatOptions.WxOpenAppId}") || string.IsNullOrEmpty(_cacheManager.Get<string>($"WxAccessToken_{_wechatOptions.WxOpenAppId}")))
        {
            ...
            if (resCgibinToken.ErrorCode != (int)WechatReturnCodeEnum.请求成功)
                throw Oops.Oh(resCgibinToken.ErrorMessage + " " + resCgibinToken.ErrorCode);
            _cacheManager.Set($"WxAccessToken_{_wechatOptions.WxOpenAppId}", resCgibinToken.AccessToken, TimeSpan.FromSeconds(resCgibinToken.ExpiresIn - 60));
        }

        return _cacheManager.Get<string>($"WxAccessToken_{_wechatOptions.WxOpenAppId}");
    }
    ...
    public async Task CheckWechatAccessTokenAsync()
    {
        ...
        {
            _cacheManager.Remove($"WxAccessToken_{_wechatOptions.WechatAppId}");
    ...
    public async Task CheckWxOpenAccessTokenAsync()
    {
        ...
        if (res.ErrorCode != (int)WechatReturnCodeEnum.请求成功)
        {
            _cacheManager.Remove($"WxAccessToken_{_wechatOptions.WxOpenAppId}");
            Console.ForegroundColor = ConsoleColor.Red;
    ...
    {
        if (!_cacheManager.ExistKey($"WxJsApiTicket_{_wechatOptions.WechatAppId}") || string.IsNullOrEmpty(_cacheManager.Get<string>($"WxJsApiTicket_{_wechatOptions.WechatAppId}")))
        {
            ...
            _cacheManager.Set($"WxJsApiTicket_{_wechatOptions.WechatAppId}", response.Ticket, TimeSpan.FromSeconds(response.ExpiresIn - 60));
        }
        return _cacheManager.Get<string>($"WxJsApiTicket_{_wechatOptions.WechatAppId}");
    }
}