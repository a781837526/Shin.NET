...
public class SysWechatPayService : ISysWechatPayService
{
    /// <summary>
    /// 基础仓储服务
    /// </summary>
    private readonly ISqlSugarRepository<SysWechatPay> _repository;
    ...
    /// <summary>
    /// 初始化<see cref="SysWechatPayService"/>类的新实例
    /// </summary>
    public SysWechatPayService(ISqlSugarRepository<SysWechatPay> repository,
    ...
    {
        return await _repository.Context.Queryable<SysWechatRefund>().Where(u => u.TransactionId == id).ToListAsync();
    }
    ...
    public async Task<WechatPayTransactionOutput> CreatePayTransaction([FromBody] WechatPayTransactionInput input)
    {
        ...
        await _repository.InsertAsync(wechatPay);
    ...
    {
        return await _repository.GetFirstAsync(u => u.OutTradeNumber == tradeId);
    }
    ...
    public async Task<SysWechatPay> GetPayInfoFromWechat(string tradeId)
    {
        ...
        // 修改订单支付状态
        var wechatPay = await _repository.GetFirstAsync(u => u.OutTradeNumber == response.OutTradeNumber
        ...
        if (wechatPay != null && wechatPay.TradeState != response.TradeState)
        {
            ...
            await _repository.AsUpdateable(wechatPay).IgnoreColumns(true).ExecuteCommandAsync();
        }
    ...
    public async Task<dynamic> CreateRefundDomestic([FromBody] WechatPayRefundDomesticInput input)
    {
        ...
        if (string.IsNullOrEmpty(response.ErrorCode))
        {
            ...
            if (wechatPay != null)
            {
                ...
                await _repository.Context.Insertable(wechatRefund).ExecuteCommandAsync();
            }
        }
        else
        {
    ...
    public async Task<SysWechatRefund> GetRefundInfoFromWechat(string refundId)
    {
        ...
        // 修改订单支付状态
        var wechatRefund = await _repository.Context.Queryable<SysWechatRefund>().FirstAsync(u => u.OutRefundNumber == refundId);
        // 如果状态不一致就更新数据库中的记录
        if (wechatRefund != null && wechatRefund.TradeState != response.Status)
        {
            ...
            await _repository.Context.Updateable(wechatRefund).IgnoreColumns(true).ExecuteCommandAsync();
            // 有退款，刷新一下订单状态
            var wechatPay = await _repository.GetFirstAsync(u => u.Id == wechatRefund.WechatPayId);
            if (wechatPay != null)
    ...
    public async Task<WechatPayOutput> PayCallBack()
    {
        ...
        if ("TRANSACTION.SUCCESS".Equals(callbackModel.EventType))
        {
            try
            {
                ...
                // 修改订单支付状态
                var wechatPay = await _repository.GetFirstAsync(u => u.OutTradeNumber == callbackPayResource.OutTradeNumber
        ...
        else if ("REFUND.SUCCESS".Equals(callbackModel.EventType))
        {
            ...
            try
            {
                ...
                // 修改订单支付状态
                var wechatRefund = await _repository.Context.Queryable<SysWechatRefund>().FirstAsync(u => u.OutRefundNumber == callbackRefundResource.OutRefundNumber);
    ...
    public async Task PayPartnerCallBack()
    {
        ...
        if ("TRANSACTION.SUCCESS".Equals(callbackModel.EventType))
        {
            ...
            // 修改订单支付状态
            var wechatPay = await _repository.GetFirstAsync(u => u.OutTradeNumber == callbackResource.OutTradeNumber
