...
public class SysUpdateService : ISysUpdateService
{
    /// <summary>
    /// 缓存管理
    /// </summary>
    ...
    /// <summary>
    /// 初始化<see cref="SysUpdateService"/>类的新实例
    /// </summary>
    public SysUpdateService(IOptions<CDConfigOptions> giteeOptions,
    ...
    {
        const string backendDir = "Shin.NET";
    ...
    public async Task Update()
    {
        ...
        try
        {
            ...
            {
                if (_cacheManager.Get<bool>(CacheConst.KeySysUpdateInterval)) throw Oops.Oh("请勿频繁操作");
                _cacheManager.Set(CacheConst.KeySysUpdateInterval, true, TimeSpan.FromMinutes(_cdConfigOptions.UpdateInterval));
            }
            ...
            // 项目目录
            var backendDir = "Shin.NET"; // 后端根目录
            var entryProjectName = "Shin.Web.Entry"; // 启动项目目录
    ...
    {
        return _cacheManager.Get<List<string>>(CacheConst.KeySysUpdateLog) ?? new();
    }
    ...
    {
        _cacheManager.Remove(CacheConst.KeySysUpdateLog);
    }
    ...
    {
        var logList = _cacheManager.Get<List<string>>(CacheConst.KeySysUpdateLog) ?? new();
        ...
        _cacheManager.Set(CacheConst.KeySysUpdateLog, logList);
    }
