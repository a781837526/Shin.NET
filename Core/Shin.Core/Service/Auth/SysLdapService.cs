...
public class SysLdapService : ISysLdapService
{
    /// <summary>
    /// 基础服务仓储
    /// </summary>
    private readonly ISqlSugarRepository<SysLdap> _repository;

    /// <summary>
    /// 初始化<see cref="SysLdapService"/>类的新实例
    /// </summary>
    public SysLdapService(ISqlSugarRepository<SysLdap> repository)
    {
    ...
    {
        return await _repository.AsQueryable().Select<SysLdap>().ToListAsync();
    }
    ...
    {
        var sysLdap = await _repository.GetFirstAsync(u => u.TenantId == tenantId) ?? throw Oops.Oh(ErrorCodeEnum.D1002);
    ...
    {
        var sysLdap = await _repository.GetFirstAsync(c => c.TenantId == tenantId && c.IsDelete == false && c.Status == StatusEnum.Enable) ?? throw Oops.Oh(ErrorCodeEnum.D1002);
    ...
    {
        var sysLdap = await _repository.GetByIdAsync(input.Id) ?? throw Oops.Oh(ErrorCodeEnum.D1002);
        return await SysLdapService.SyncUser(sysLdap);
    }
    ...
    {
        var sysLdap = await _repository.GetFirstAsync(u => u.Id == input.Id) ?? throw Oops.Oh(ErrorCodeEnum.D1002);
        var ldapConn = new LdapConnection();
        try
        {
            await ldapConn.ConnectAsync(sysLdap.Host, sysLdap.Port);
