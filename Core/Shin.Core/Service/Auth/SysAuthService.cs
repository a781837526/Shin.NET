...
public class SysAuthService : ISysAuthService
{
    ...
    /// 系统短信服务
    /// </summary>
    private readonly ISysSmsService _sysSmsService;

    /// <summary>
    /// 系统域登录配置服务
    /// </summary>
    private readonly ISysLdapService _sysLdapService;

    /// <summary>
    /// 初始化<see cref="SysAuthService"/>类的新实例
    /// </summary>
    public SysAuthService(
        ISqlSugarRepository<SysUser> repository,
        ...
        ISysLdapService sysLdapService,
    ...
    public virtual async Task<LoginOutput> Login([Required] LoginInput input)
    {
        ...
        var passwordErrorTimes = _cacheManager.Get<int>(keyPasswordErrorTimes);
        ...
        {
            var userLdap = await _repository.ChangeRepository<SqlSugarRepository<SysUserLdap>>().GetFirstAsync(u => u.UserId == user.Id && u.TenantId == tenant.Id);
            if (userLdap == null)
            {
            ...
            {
                _cacheManager.Set(keyPasswordErrorTimes, ++passwordErrorTimes, TimeSpan.FromMinutes(30));
    ...
    {
        // 账号是否存在
        var user = await _repository.AsQueryable().Includes(u => u.SysOrg).ClearFilter()
        ...
        // 租户是否存在或已禁用
        var tenant = await _repository.Context.Queryable<SysTenant>()
    ...
    {
        // 账号是否存在
        var user = await _repository.GetFirstAsync(u => u.Id == _userManager.UserId);
        ...
        if (await _sysConfigService.GetConfigValue<bool>(ConfigConst.SysDomainLogin))
        {
            var userLdap = await _repository.ChangeRepository<SqlSugarRepository<SysUserLdap>>().GetFirstAsync(u => u.UserId == user.Id && u.TenantId == user.TenantId);
            if (userLdap == null)
            {
    ...
    internal virtual async Task<LoginOutput> CreateToken(SysUser user, SysUserEventTypeEnum sysUserEventTypeEnum = SysUserEventTypeEnum.Login)
    {
        ...
        await _repository.AsUpdateable(user).UpdateColumns(u => new
        {
            u.LastLoginIp,
            u.LastLoginAddress,
            u.LastLoginTime,
    ...
    public virtual async Task<LoginUserOutput> GetUserInfo()
    {
        ...
        // 获取机构
        var org = await _repository.ChangeRepository<SqlSugarRepository<SysOrg>>().GetFirstAsync(u => u.Id == user.OrgId);
        // 获取职位
        var pos = await _repository.ChangeRepository<SqlSugarRepository<SysPos>>().GetFirstAsync(u => u.Id == user.PosId);
        // 获取按钮集合
        var buttons = await _sysMenuService.GetOwnBtnPermList();
        // 获取角色集合
        var roleIds = await _repository.ChangeRepository<SqlSugarRepository<SysUserRole>>().AsQueryable()
            .Where(u => u.UserId == user.Id).Select(u => u.RoleId).ToListAsync();
        // 获取水印文字（若系统水印为空则全局为空）
        var watermarkText = (await _repository.Context.Queryable<SysTenant>().FirstAsync(u => u.Id == user.TenantId))?.Watermark;
    ...
    public virtual async Task<LoginOutput> GetRefreshToken([FromQuery] string accessToken)
    {
        ...
        // 黑名单校验
        if (_cacheManager.ExistKey($"blacklist:token:{accessToken}")) throw Oops.Oh(ErrorCodeEnum.D1011);
    ...
    public async Task Logout()
    {
        ...
        var tokenExpire = await _sysConfigService.GetTokenExpire();
        _cacheManager.Set($"blacklist:token:{token}", "1", TimeSpan.FromMinutes(tokenExpire));

        // 发布登出事件（用户退出）
        var user = await _repository.GetByIdAsync(_userManager.UserId);
    ...
    public async Task UserRegistration(UserRegistrationInput input)
    {
        ...
        // 判断租户是否有效且启用注册功能
        var tenant = await _repository.Context.Queryable<SysTenant>().FirstAsync(u => u.Id == input.TenantId && u.Status == StatusEnum.Enable);
        if (tenant?.EnableReg != YesNoEnum.Y) throw Oops.Oh(ErrorCodeEnum.D1034);

        // 查找注册方案
        var wayId = input.WayId <= 0 ? tenant.RegWayId : input.WayId;
        var regWay = await _repository.Context.Queryable<SysUserRegWay>().FirstAsync(u => u.Id == wayId) ?? throw Oops.Oh(ErrorCodeEnum.D1035);
