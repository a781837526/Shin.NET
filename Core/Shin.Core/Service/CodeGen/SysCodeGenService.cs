...
public class SysCodeGenService : ISysCodeGenService
{
    /// <summary>
    /// 数据库配置选项
    /// </summary>
    ...
    /// 系统代码生成配置服务
    /// </summary>
    private readonly ISysCodeGenConfigService _codeGenConfigService;
    ...
    public async Task<dynamic> RunLocal(SysCodeGen input)
    {
        ...
        {
            var content = result.GetValueOrDefault(templatePathList[i]?.TrimSuffix(".vm"));
    ...
    private async Task<(List<CodeGenConfig> tableFieldList, Dictionary<string, string> result)> RenderTemplateAsync(SysCodeGen input)
    {
        ...
        foreach (var path in templatePathList)
        {
            ...
            result.Add(path?.TrimSuffix(".vm"), tResult);
        }
