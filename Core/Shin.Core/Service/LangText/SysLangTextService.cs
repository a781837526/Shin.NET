...
public partial class SysLangTextService : ISysLangTextService
{
    /// <summary>
    /// 导入锁
    /// </summary>
    ...
    /// <summary>
    /// 初始化<see cref="SysLangTextService"/>类的新实例
    /// </summary>
    ...
    public async Task BatchDelete([Required(ErrorMessage = "主键列表不能为空")] List<DeleteSysLangTextInput> input)
    {
        ...
        var list = await _repository.AsQueryable().Where(exp.ToExpression()).ToListAsync();

        await _repository.DeleteAsync(list);   //真删除
    ...
    public void BatchSave([Required(ErrorMessage = "列表不能为空")] List<ImportSysLangTextInput> input)
    {
        lock (_sysLangTextBatchSaveLock)
        {
            ...
            var storageable = _repository.Context.Storageable(rows)
