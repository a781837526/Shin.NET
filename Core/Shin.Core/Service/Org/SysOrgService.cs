...
public class SysOrgService : ISysOrgService
{
    ...
    /// <summary>
    /// 初始化<see cref="SysOrgService"/>类的新实例
    /// </summary>
    public SysOrgService(ISqlSugarRepository<SysOrg> repository,
    ...
    public async Task<List<SysOrg>> GetList([FromQuery] OrgInput input)
    {
        ...
        var queryable = _repository.AsQueryable().WhereIF(input.TenantId > 0, u => u.TenantId == input.TenantId).OrderBy(u => new { u.OrderNo, u.Id });
        // 带条件筛选时返回列表数据
        ...
        var sysOrg = await _repository.GetSingleAsync(u => u.Id == input.Id);
    ...
    public async Task<List<OrgTreeOutput>> GetTree([FromQuery] OrgInput input)
    {
        ...
        var sysOrg = await _repository.AsQueryable().Select<OrgTreeOutput>().FirstAsync(u => u.Id == input.Id);
        if (sysOrg == null) return orgTree;
    ...
    public async Task<long> AddOrg(AddOrgInput input)
    {
        ...
        {
            var pOrg = await _repository.GetFirstAsync(u => u.Id == input.Pid);
            if (pOrg != null)
                DeleteAllUserOrgCache(pOrg.Id, pOrg.Pid);
        }

        var newOrg = await _repository.AsInsertable(input.Adapt<SysOrg>()).ExecuteReturnEntityAsync();
        return newOrg.Id;
    }
    ...
    {
        DeleteAllUserOrgCache(0, 0);
        await _repository.AsDeleteable().ExecuteCommandAsync();
        await _repository.AsInsertable(orgs).ExecuteCommandAsync();
    }
    ...
    public async Task UpdateOrg(UpdateOrgInput input)
    {
        ...
        if (input.Pid != 0)
        {
            ...
            // 若父机构发生变化则清空用户机构缓存
            var sysOrg = await _repository.GetFirstAsync(u => u.Id == input.Id);
        ...
        await _repository.AsUpdateable(input.Adapt<SysOrg>()).IgnoreColumns(true).ExecuteCommandAsync();
    }
    ...
    {
        var sysOrg = await _repository.GetFirstAsync(u => u.Id == input.Id) ?? throw Oops.Oh(ErrorCodeEnum.D1002);
        ...
        // 若机构为租户默认机构禁止删除
        var isTenantOrg = await _repository.ChangeRepository<SqlSugarRepository<SysTenant>>()
            .IsAnyAsync(u => u.OrgId == input.Id);
        if (isTenantOrg)
            throw Oops.Oh(ErrorCodeEnum.D2008);

        // 若机构有用户则禁止删除
        var orgHasEmp = await _repository.ChangeRepository<SqlSugarRepository<SysUser>>()
            .IsAnyAsync(u => u.OrgId == input.Id);
        if (orgHasEmp)
        ...
        // 若子机构有用户则禁止删除
        var childOrgTreeList = await _repository.AsQueryable().ToChildListAsync(u => u.Pid, input.Id, true);
        var childOrgIdList = childOrgTreeList.Select(u => u.Id).ToList();

        // 若子机构有用户则禁止删除
        var cOrgHasEmp = await _repository.ChangeRepository<SqlSugarRepository<SysUser>>()
            .IsAnyAsync(u => childOrgIdList.Contains(u.OrgId));
        if (cOrgHasEmp) throw Oops.Oh(ErrorCodeEnum.D2007);

        // 若有绑定注册方案则禁止删除
        var hasUserRegWay = await _repository.Context.Queryable<SysUserRegWay>().AnyAsync(u => u.OrgId == input.Id);
        ...
        // 级联删除机构子节点
        await _repository.DeleteAsync(u => childOrgIdList.Contains(u.Id));
    ...
    {
        var orgIdList = _cacheManager.Get<List<long>>($"{CacheConst.KeyUserOrg}{userId}"); // 取缓存
        if (orgIdList is { Count: >= 1 }) return orgIdList;

        // 本人创建机构集合
        var orgList0 = await _repository.AsQueryable().Where(u => u.CreateUserId == userId).Select(u => u.Id).ToListAsync();
        ...
        _cacheManager.Set($"{CacheConst.KeyUserOrg}{userId}", orgIdList, TimeSpan.FromDays(7)); // 存缓存
        return orgIdList;
    }
    ...
    {
        var orgTreeList = await _repository.AsQueryable().ToChildListAsync(u => u.Pid, pid, true);
    ...
    {
        var userOrgKeyList = _cacheManager.GetKeysByPrefixKey(CacheConst.KeyUserOrg);
        if (userOrgKeyList is not { Count: > 0 }) return;

        foreach (var userOrgKey in userOrgKeyList)
        {
            var userOrgList = _cacheManager.Get<List<long>>(userOrgKey);
            ...
            var dataScope = _cacheManager.Get<int>($"{CacheConst.KeyRoleMaxDataScope}{userId}");
    ...
    private async Task<List<long>> GetUserOrgIdList(List<SysRole> roleList, long userId, long userOrgId)
    {
        ...
        // 缓存当前用户最大角色数据范围
        _cacheManager.Set(CacheConst.KeyRoleMaxDataScope + userId, strongerDataScopeType, TimeSpan.FromDays(7));
    ...
    private async Task<List<long>> GetOrgIdListByDataScope(long userOrgId, int dataScope)
    {
        ...
        switch (dataScope)
        {
            ...
            case (int)DataScopeEnum.All:
                orgIdList = await _repository.AsQueryable().Select(u => u.Id).ToListAsync();
                break;
