...
{
    private static readonly ICacheManager cacheManager = App.GetRequiredService<ICacheManager>();
    private static readonly ISysFileService SysFileService = App.GetRequiredService<ISysFileService>();
    private static readonly ISqlSugarRepository<SysDictData> SysDictDataRep = App.GetRequiredService<ISqlSugarRepository<SysDictData>>();
    ...
    public static async Task<ICollection<T>> ImportExcelData<T>([Required] IFormFile file, Func<ImportResult<T>, ImportResult<T>> importResultCallback = null) where T : class, new()
    {
        ...
        cacheManager.Remove(CacheConst.KeyExcelTemp + userId);
        cacheManager.Set(CacheConst.KeyExcelTemp + userId, resultStream, TimeSpan.FromMinutes(5));
    ...
    private static Dictionary<string, Tuple<Dictionary<string, object>, PropertyInfo, PropertyInfo>> GetImportPropertMap<TSource, TTarget>() where TTarget : new()
    {
        ...
        var dictService = App.GetRequiredService<ISqlSugarRepository<SysDictData>>();
